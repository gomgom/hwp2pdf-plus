...
## 실행이 막힐 때 — 스마트 앱 컨트롤 끄기

Windows 11에서 **"스마트 앱 컨트롤이 안전하지 않을 수 있는 앱을 차단했습니다"** 라며 실행이 막히고 *[계속 실행]* 같은 버튼도 보이지 않는다면, **스마트 앱 컨트롤(Smart App Control)** 기능 때문입니다. 서명되지 않은 개인 배포 앱이라 생기는 것으로, 아래처럼 잠깐 껐다가 실행한 뒤 다시 켜면 됩니다.

1. **시작 버튼**(또는 검색)을 누르고 **`스마트 앱 컨트롤`** 을 검색해 여세요.

   ※ **설정 → 개인정보 및 보안 → Windows 보안 → 앱 및 브라우저 컨트롤 → 스마트 앱 컨트롤 → 스마트 앱 컨트롤 설정 → 끄기** 를 통해서도 접근할 수 있습니다.

   <img src="res/SAC_off_1.png" width="480" alt="시작에서 '스마트 앱 컨트롤' 검색">

2. **`끄기`** 를 선택합니다.

   <img src="res/SAC_off_2.png" width="480" alt="스마트 앱 컨트롤 '끄기' 선택">

3. `hwp2pdf-plus.exe` 를 실행합니다. 실행 후에는 보안을 위해 다시 **`켬`** 으로 돌려두는 것을 권장합니다.

> 2026년 4월 Windows 업데이트부터는 스마트 앱 컨트롤을 껐다가 다시 켤 수 있습니다. 그 이전 버전에서는 한 번 끄면 되돌리기 어려우니, **먼저 Windows를 최신으로 업데이트**한 뒤 진행하세요.
